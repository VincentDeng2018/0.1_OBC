...
{
#ifdef DEBUG_TEST
    static USHORT TestOneTime = 1u;
    UINT8 TestData[8u] = {0x40, 0u, 0x10, 0u, 0u, 0u, 0u, 0u};
    ...
    if(TestOneTime)
    {
        ...
        memcpy(&ReceiveData[0].Data[0], &TestData[0u], 8u);
        memcpy(&ReceiveData[1].Data[0], &TestData[0u], 8u);
        memcpy(&ReceiveData[2].Data[0], &TestData[0u], 8u);
        
        ReceiveData[0].ID = 0x80;
        ReceiveData[0].DataLen = 0x8u;
        
        ReceiveData[1].ID = 0x601;
        ReceiveData[1].DataLen = 0x8u;
        
        ReceiveData[2].ID = 0x181;
        ReceiveData[2].DataLen = 0x8u;
        return;
    }
#endif
    if(ConnectAndStart)
