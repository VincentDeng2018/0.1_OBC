...
/***************** Application ******************/
/* battery status */
uint8_t OBJ6000_BattStatus = 0xFFu;
...
/* charger status */
uint8_t OBJ6001_ChargerStatus = 0xFFu;

stOdSubInfo_t stObIndex6001[] =
{
    { RW, UNSIGNED8, sizeof (uint8_t), (void*)&OBJ6001_ChargerStatus }
};
...
uint16_t OBJ60A1_SystemState = 0u;
...
void UpdateChargerOd(void)
{
    ...
    /* 60A2h  */
    OBJ60A2_PwmState = ((uint32_t)PWM_UCmdLast_x10 << 16u) + (uint32_t)PWM_ICmdLast_x10;
...
{
    return (int16_t)(OBJ6010_BattTemperature * 0.8);
}
...
#else
    OBJ6010_BattTemperature = (int16_t)((uint16_t)(pData[1] << 8) | pData[0]);
    OBJ6000_BattStatus = pData[2];
    
    OBJ6060_BatteryVolts_Q10V = (uint32_t)pData[3] 
                              | ((uint32_t)pData[4] << 8u)
                              | ((uint32_t)pData[5] << 16u)
                              | ((uint32_t)pData[6] << 24u);
#endif
}
...
#else
    OBJ6070_ChargeCurrntRequest_Q4A = (uint16_t)((uint16_t)(pData[1] << 8) | pData[0]);
