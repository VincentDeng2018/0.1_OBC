    ...
    while(1)
    {
        ...
        if(CanDriverObj->ReceiveLength)
        {
            for(i = 0; i<CanDriverObj->ReceiveLength; i++)
            {
                ...
                while(MainForm->CanFrameGridBusy)
                {
                    Sleep(1);
                }
                MainForm->InsertOneCanFrame(&CanDriverObj->ReceiveData[i]);
                ...
                str+=tmpstr;
                box->Items->Add(str);
                fprintf(CanDriverObj->stream,  str.c_str());
                fprintf(CanDriverObj->stream,  "\r\n");
                if(CanDriverObj->ReceiveData[i].RemoteFlag==0)
                {
                    str="数据:";
...
UINT16 CanProtocol::XcpSendData(stCanMessage_t *pCanMsg)
{
    ...
    MainForm->InsertOneCanFrame(&RequestCanMsg);
