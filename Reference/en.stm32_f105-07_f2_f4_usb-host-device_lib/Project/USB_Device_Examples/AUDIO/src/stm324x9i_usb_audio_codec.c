...
/* File Info :
 * ------------------------------------------------------------------------------------------
 * User NOTES 1. How To use this driver: -------------------------- - This driver supports
 * STM32F4xx devices on STM324x9I-EVAL (MB1045) Evaluation boards.
 * 
 ...
 * 
 * - Call the function EVAL_AUDIO_Init( OutputDevice : physical output mode
 * (OUTPUT_DEVICE_SPEAKER, OUTPUT_DEVICE_HEADPHONE or OUTPUT_DEVICE_BOTH) Volume : Initial
 * volume to be set (0 is min (mute), 100 is max (100%) AudioFreq : Audio frequency in Hz
 * (8000, 16000, 22500, 32000...) this parameter is relative to the audio file/stream type. )
 * This function configures all the hardware required for the audio application (codec, I2C,
 * SAI, GPIOs, DMA and interrupt if needed). This function returns 0 if configuration is OK.
 * if the returned value is different from 0 or the function is stuck then the communication
 * with the codec or the IOExpander has failed (try to un-plug the power or reset device in
 * this case). + OUTPUT_DEVICE_SPEAKER : only speaker will be set as output for the audio
 * stream. + OUTPUT_DEVICE_HEADPHONE: only headphones will be set as output for the audio
 * stream. + OUTPUT_DEVICE_BOTH : both Speaker and Headphone are used as outputs for the audio 
 * stream at the same time.
 * 
 * - Call the function EVAL_AUDIO_Play( pBuffer : pointer to the audio data file address Size
 * : size of the buffer to be sent in Bytes ) to start playing (for the first time) from the
 * audio file/stream.
 * 
 * - Call the function EVAL_AUDIO_PauseResume( Cmd: AUDIO_PAUSE (or 0) to pause playing or
 * AUDIO_RESUME (or any value different from 0) to resume playing. ) Note. After calling
 * EVAL_AUDIO_PauseResume() function for pause, only EVAL_AUDIO_PauseResume() should be called
 * for resume (it is not allowed to call EVAL_AUDIO_Play() in this case). Note. This function
 * should be called only when the audio file is played or paused (not stopped).
 * 
 * - For each mode, you may need to implement the relative callback functions into your code.
 * The Callback functions are named EVAL_AUDIO_XXX_CallBack() and only their prototypes are
 * declared in the STM324x9i_eval_audio_codec.h file. (refer to the example for more details
 ...
 * 
 * 
 * Driver architecture: -------------------- This driver is composed of three main layers: o
 * High Audio Layer: consists of the function API exported in the STM324x9i_eval_audio_codec.h
 * file (EVAL_AUDIO_Init(), EVAL_AUDIO_Play() ...) o Codec Control layer: consists of the
 * functions API controlling the audio codec (WM8994) and included as local functions in file
 * STM324x9i_eval_audio_codec.c (Codec_Init(), Codec_Play() ...) o Media Access Layer (MAL):
 * which consists of functions allowing to access the media containing/ providing the audio
 * file/stream. These functions are also included as local functions into the
 * STM324x9i_eval_audio_codec.c file (Audio_MAL_Init(), Audio_MAL_Play() ...) Each set of
 * functions (layer) may be implemented independently of the others and customized when
 * needed.
 * 
 * 2. Modes description: --------------------- + AUDIO_MAL_MODE_NORMAL : is suitable when the
 * audio file is in a memory location. + AUDIO_MAL_MODE_CIRCULAR: is suitable when the audio
 * data are read either from a memory location or from a device at real time (double buffer
 * could be used).
 * 
 * 3. DMA interrupts description: ------------------------------ + EVAL_AUDIO_IT_TC_ENABLE:
 * Enable this define to use the DMA end of transfer interrupt. then, a callback should be
 * implemented by user to perform specific actions when the DMA has finished the transfer. +
 * EVAL_AUDIO_IT_HT_ENABLE: Enable this define to use the DMA end of half transfer interrupt.
 * then, a callback should be implemented by user to perform specific actions when the DMA has
 * reached the half of the buffer transfer (generally, it is useful to load the first half of
 * buffer while DMA is loading from the second half). + EVAL_AUDIO_IT_ER_ENABLE: Enable this
 * define to manage the cases of error on DMA transfer.
 * 
 * 4. Known Limitations: ---------------------
 * 
 * 1- If the TDM Format used to paly in parallel 2 audio Stream (the first Stream is configured 
 * in codec SLOT0 and second Stream in SLOT1) the Pause/Resume, volume and mute feature will
 * control the both streams.
 * 
 * 2- Parsing of audio file is not implemented (in order to determine audio file properties:
 * Mono/Stereo, Data size, File size, Audio Frequency, Audio Data header size ...). The
 * configuration is fixed for the given audio file.
 * 
...
static uint32_t Codec_WriteRegister(uint16_t RegisterAddr,
                         ...
                         uint32_t AudioFreq)
{
  ...
  {
    /* SAI data transfer preparation: Prepare the Media to be used for the
     * audio transfer from memory to SAI peripheral */
    Audio_MAL_Init();
                           ...
                           uint32_t AudioFreq)
{
  ...
  case OUTPUT_DEVICE_BOTH:
    /* Enable DAC1 (Left), Enable DAC1 (Right), also Enable DAC2 (Left),
     * Enable DAC2 (Right) */
  ...
  case OUTPUT_DEVICE_SPEAKER:
    /* Enable DAC1 (Left), Enable DAC1 (Right), Disable DAC2 (Left), Disable
     * DAC2 (Right) */
  ...
  case OUTPUT_DEVICE_HEADPHONE:
    /* Disable DAC1 (Left), Disable DAC1 (Right), Enable DAC2 (Left), Enable
     * DAC2 (Right) */
  ...
  /* Speaker output mode = Class D, Right Speaker Mixer Volume = 0dB ((0x23,
   * 0x0100) = class AB) */
  ...
  /* Unmute DAC2 (Left) to Left Speaker Mixer (SPKMIXL) path, Unmute DAC2
   * (Right) to Right Speaker Mixer (SPKMIXR) path */
  ...
  /* Enable bias generator, Enable VMID, Enable HPOUT1 (Left) and Enable HPOUT1 
   * (Right) input stages */
  /* idem for Speaker */
...
static void Codec_CtrlInterface_DeInit(void)
{
  ...
  /* This step is not done here because the I2C interface can be used by other
   * modules */
  /* I2C_DeInit(CODEC_I2C); */
}
...
static void Codec_AudioInterface_Init(uint32_t AudioFreq)
{
  ...
  /* Configure Master Clock using the following formula : MCLK_x = SAI_CK_x /
   * (MCKDIV[3:0] * 2) with MCLK_x = 256 * FS FS = SAI_CK_x / (MCKDIV[3:0] * 2) 
   * * 256 MCKDIV[3:0] = SAI_CK_x / FS * 512 */
  ...
  SAI_InitStructure.SAI_SynchroExt = SAI_SyncExt_Disable;
  ...
  /* Configure SAI_Block_x Frame Frame Length : 32 Frame active Length: 16 FS
   * Definition : Start frame + Channel Side identification FS Polarity: FS
   * active Low FS Offset: FS asserted one bit before the first bit of slot 0 */
  ...
  /* Configure SAI Block_x Slot Slot First Bit Offset : 0 Slot Size : 16 Slot
   * Number : 4 Slot Active : All slot actives */
...
static void Codec_GPIO_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin =
    CODEC_SAI_FS_PIN | CODEC_SAI_SD_PIN | CODEC_SAI_SCK_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
...
static void Codec_GPIO_DeInit(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin =
    CODEC_SAI_FS_PIN | CODEC_SAI_SD_PIN | CODEC_SAI_SCK_PIN | CODEC_SAI_MCK_PIN;
...
uint32_t Codec_TIMEOUT_UserCallback(void)
{
   ...
   * again (except if the error source still exist). User can implement
   * mechanism (ex. test on max trial number) to manage situation when the I2C
   * can't recover from current error. */
...
static void Audio_MAL_Init(void)
{
  ...
  DMA_InitStructure.DMA_BufferSize = (uint32_t) 0xFFFF; /* This field will be
                                                         * configured in play
                                                         * function */
...
static void Audio_MAL_PauseResume(uint32_t Cmd, uint32_t Addr)
{
  ...
  if (Cmd == AUDIO_PAUSE)
  {
    ...
    /* Pause the SAI DMA Stream Note. For the STM32F4xx devices, the DMA
  ...
  else                          /* AUDIO_RESUME */
  {
    ...
    /* Resume the SAI DMA Stream Note. For the STM32F4xx devices, the DMA
     * implements a pause feature, by disabling the stream, all configuration
     * is preserved and data transfer is paused till the next enable of the
     * stream. */
    DMA_Cmd(AUDIO_MAL_DMA_STREAM, ENABLE);
