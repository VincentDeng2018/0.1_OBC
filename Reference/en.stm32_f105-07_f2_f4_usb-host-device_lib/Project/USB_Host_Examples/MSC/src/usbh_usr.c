...
#error "Missing define: Evaluation board (ie. USE_STM322xG_EVAL)"
  ...
  LCD_LOG_SetHeader((uint8_t *) " USB OTG HS_IN_FS MSC Host");
  #else
...
void USBH_USR_EnumerationDone(void)
{
  ...
  LCD_DisplayStringLine(LCD_PIXEL_HEIGHT - 42,
                        (uint8_t *) "To see the root content of the disk : ");
  LCD_DisplayStringLine(LCD_PIXEL_HEIGHT - 30,
                        (uint8_t *) "Press Key...                       ");
...
int USBH_USR_MSC_Application(void)
{
  ...
  case USH_USR_FS_WRITEFILE:
    ...
    LCD_DisplayStringLine(LCD_PIXEL_HEIGHT - 42,
                          ...
                          "                                              ");
    LCD_DisplayStringLine(LCD_PIXEL_HEIGHT - 30,
                          (uint8_t *) "Press Key to write file");
    LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR);
    ...
    LCD_DisplayStringLine(LCD_PIXEL_HEIGHT - 30,
                          (uint8_t *) "To start Image slide show Press Key.");
#endif
    LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR);
...
static uint8_t Image_Browser(char *path)
{
  ...
  LCD_LOG_SetHeader((uint8_t *) " USB OTG FS MSC Host");
#endif
  LCD_LOG_SetFooter((uint8_t *) "     USB Host Library v2.2.0");
  LCD_UsrLog("> Disk capacity : %lu Bytes\n", USBH_MSC_Param.MSCapacity *
             USBH_MSC_Param.MSPageLength);
  USBH_USR_ApplicationState = USH_USR_FS_READLIST;
  return ret;
