...
#error "Missing define: Evaluation board (ie. USE_STM322xG_EVAL)"
  ...
  USBH_Init(&USB_OTG_Core,
            USB_OTG_HS_CORE_ID, &USB_Host, &USBH_MSC_cb, &USR_MSC_cb);
  LCD_UsrLog("USB Host High speed initialized.\n");
  /* Init FS Core */
  USBH_Init(&USB_OTG_FS_Core,
            USB_OTG_FS_CORE_ID, &USB_FS_Host, &HID_cb, &USR_HID_cb);
...
static void Demo_Application(void)
{
  ...
  if (!(HCD_IsDeviceConnected(&USB_OTG_Core)) && (HS_DemoDisconnHandled == 0))
  {
    ...
    for (;;)
    {
      ...
      else
      {
        if ((strstr(fn, "bmp")) || (strstr(fn, "BMP")))
        {
          ...
          Show_Image();
          wait_user_input = 1;
          LCD_DisplayStringLine(LCD_PIXEL_HEIGHT - 12,
                                (uint8_t *)
...
static uint8_t Check_BMP_file(uint8_t * buf)
{
  ...
  if ((Type != BF_TYPE) ||
      (Width != 320) ||
      (Height != 240) || (Bpp != 16) || (Compression != BI_BITFIELDS))
  {
    return 1;
  }
