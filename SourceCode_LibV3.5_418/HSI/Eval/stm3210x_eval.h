...
#ifndef __STM3210X_EVAL_H
#define __STM3210X_EVAL_H
...
typedef __packed struct Hid_s
 {
  uint8_t flag;            // byte was single byte with 8 bit length
  uint8_t Command;   
  uint8_t month;
  uint8_t day;
  uint16_t year;
...
void EmptyReceiveFrame(void);
void UpdateRegisters(void);

void Transmit(void);

void Rx_From_Uart(void);
void Send_To_Uart(void);
void mcp_test(void);
void mcp_rtc_test(Hid_t *pHid);
