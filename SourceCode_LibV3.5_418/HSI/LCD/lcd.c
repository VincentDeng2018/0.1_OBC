...
#define  SDA_HIGH   GPIO_SetBits(SPI_MOSI_PORT, SPI_MOSI_PIN)
#define  SDA_LOW    GPIO_ResetBits(SPI_MOSI_PORT, SPI_MOSI_PIN)
#define  SCL_HIGH   GPIO_SetBits(SPI_SCK_PORT, SPI_SCK_PIN)
#define  SCL_LOW    GPIO_ResetBits(SPI_SCK_PORT, SPI_SCK_PIN)
#define  CS_HIGH    GPIO_SetBits(SPI_CS_PORT, SPI_CS_PIN)
#define  CS_LOW     GPIO_ResetBits(SPI_CS_PORT, SPI_CS_PIN)
#define  RES_HIGH    GPIO_SetBits(LCD_RESET_PORT, LCD_RESET_PIN)
#define  RES_LOW     GPIO_ResetBits(LCD_RESET_PORT, LCD_RESET_PIN)
...
//----------------------------------------------------------------------
void  SendDataSPI(unsigned int dat)
{
    unsigned int i;
...
void WriteDispData(unsigned char DataH,unsigned char DataL)
{
    CS_LOW;
    //RS  = 1;
    //SendData_SPI_2_lane((DataH<<8)|DataL);

    SendDataSPI(0x0100|DataH);
    SendDataSPI(0x0100|DataL);
...
void LCD_Init(void)
{
    ...
    //    WriteData(0x09);  //0d
    //    WriteData(0x09);  //12
    //    WriteData(0x08);  //00  310 6
    //-------------------------------ST7789V gamma setting---------------------------------------//
    WriteComm(0xe0);// 310 7
    WriteData(0xd0);//d0
    WriteData(0x01);//0C
    WriteData(0x08);// 0F
    WriteData(0x0F);// 0B
...
void DispBand(void)
{
    ...
    for(i=0;i<8;i++)
    {
        for(j=0;j<ROW/8;j++)
        {
            for(k=0;k<COL;k++)
            {
                //SendData_SPI_2_lane(color[i]);

                SendDataSPI(0x0100|color[i]>>8);
                SendDataSPI(0x0100|color[i]);
            }
        }
    }
    for(j=0;j<ROW/8;j++)
    {
        for(k=0;k<COL;k++)
        {
            //SendData_SPI_2_lane(color[7]);
            SendDataSPI(0x0100|color[7]>>8);
            SendDataSPI(0x0100|color[7]);
        }
    }
...
void DispScaleHor1(void)
{
	...
	for(k=0;k<ROW/4;k++)
	{
		for(i=0;i<COL%32;i++){SendDataSPI(0x0100|0x00);SendDataSPI(0x0100|0x00);}
		for(i=0;i<32;i++)
		{
			for(j=0;j<COL/32;j++)
			{
				SendDataSPI(0x0100|i<<3);
				SendDataSPI(0x0100|0x00);
	...
	for(k=0;k<ROW/4;k++)
	{
		...
		for(i=0;i<32;i++)
		{
			for(j=0;j<COL/32;j++)
			{
				SendDataSPI(0x0100|(i*2)>>3);
				SendDataSPI(0x0100|(i*2)<<5);
	...
	for(k=0;k<(ROW/4+ROW%4);k++)
	{
		...
		for(i=0;i<32;i++)
		{
			for(j=0;j<COL/32;j++)
			{
				SendDataSPI(0x0100|(i<<3)|((i*2)>>3));
				SendDataSPI(0x0100|((i*2)<<5)|i);
...
//ascii 32~90(!~Z), (32~47)ｿﾕｸ/,(48~57)0~9,(58~64):~@,(65~126)A~~
...
void  DispOneChar(unsigned char ord, unsigned int Xstart, unsigned int Ystart,
...
// A~Z,ﾋﾎﾌ・2;  ｴﾋﾗﾖﾌ袞ﾂｶﾔﾓｦｵﾄｵ耻ｪ｣ｺｿ喆ｸﾟ=8x16
//(ord:10)~A
...
void Disp_GRAY_666(void)
{
    ...
    for(k=0;k<32;k++)
	{
		color=k*0x1041*2;
		//color=color&0xfefde;
		color=((color&0xfe000)>>2)|((color&0xfe00f80)>>1)|((color&0x0003e)>>1);
	for(i=0;i<ROW/32;i++)
	{
	    for(j=0;j<COL;j++)
		{
        ...
        for(i=0;i<ROW%32;i++)
	{
	    for(j=0;j<COL;j++)
		{
			SendDataSPI(0x0100|(0xffff>>8));
			SendDataSPI(0x0100|(0xffff&0xFF));
		}
	}
...
void Disp_GRAY_888(void)
{
    ...
    for(k=0;k<32;k++)
	{
		color=k*0x010101*4;
		//color=color&0xfefde;
		color=((color&0xf80000)>>8)|((color&0xfe00fc00)>>5)|((color&0x0000f8)>>3);
	for(i=0;i<ROW/32;i++)
	{
	    for(j=0;j<COL;j++)
		{
